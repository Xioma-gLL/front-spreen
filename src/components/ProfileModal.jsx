...
export default function ProfileModal({ isOpen, onClose, onLogout, initialSection = 'profile' }) {
  ...
  const renderProfileContent = () => (
    <div className="animate-fade-in pt-2">
      {/* Botón de editar alineado a la derecha, SIN el título duplicado */}
      {!isEditing && (
        <div className="flex justify-end mb-2">
            <button
                onClick={() => setIsEditing(true)}
                className="flex items-center gap-2 text-[#591117] hover:text-[#7a171f] transition-colors cursor-pointer px-2 py-1 rounded-md hover:bg-gray-50"
            >
      ...
      {/* Si se está editando, mostramos un espaciador para mantener consistencia visual */}
      {isEditing && <div className="h-4"></div>}
  ...
  const renderReservationsContent = () => (
    <div className="animate-fade-in pt-6">
       {/* Se eliminó el título duplicado "Mis Reservas" */}
  ...
  const renderNewReservationContent = () => (
    <div className="animate-fade-in pt-6">
       {/* Se eliminó el título duplicado "Nueva reserva" */}
      <div className="text-gray-500 text-center py-12 flex flex-col items-center">
  ...
  const renderMobileMenu = () => (
    <div className="flex flex-col h-full animate-fade-in">
        ...
        <nav className="flex-1 space-y-2">
            {menuItems.map((item) => (
                <button
                    ...
                    className="w-full flex items-center gap-4 px-4 py-4 rounded-xl text-left bg-gray-50 hover:bg-gray-100 active:bg-gray-200 transition-colors border border-gray-100 shadow-sm cursor-pointer"
                >
        ...
        <div className="mt-auto pt-6 border-t border-gray-100">
            <button
                onClick={onLogout}
                className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left text-red-600 hover:bg-red-50 transition-colors cursor-pointer"
            >
      ...
      >
        ...
        <div className="hidden sm:flex flex-col w-64 bg-gray-50 border-r border-gray-200 h-full">
          ...
          <div className="p-3 border-t border-gray-200 bg-gray-50">
            <button
              onClick={onLogout}
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-gray-600 hover:bg-red-50 hover:text-red-600 transition-colors group cursor-pointer"
            >
